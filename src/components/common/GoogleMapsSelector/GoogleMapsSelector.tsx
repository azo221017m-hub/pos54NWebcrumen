...
interface GoogleMapsSelectorProps {
  ...
  /** "inline" = round icon with tooltip (default, for inline use); "form" = full-width button for forms */
  variant?: 'inline' | 'form';
}

const GoogleMapsSelector: React.FC<GoogleMapsSelectorProps> = ({ value, onChange, variant = 'inline' }) => {
    ...
    <div className="gms-container">
      {!embedUrl ? (
        variant === 'form' ? (
          ...
          >
            {loading ? (
              <><span className="gms-spinner gms-spinner--dark" /> Obteniendo ubicación...</>
            ) : (
